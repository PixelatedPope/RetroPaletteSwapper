...
var _file=get_open_filename("Image File|*.bmp;*.png;*.gif;*.jpg;*jpeg", "");
if(!file_exists(_file))
    exit;
var _frames=0;
var _strip = string_count("strip",_file);
var _digits = real(string_digits(_file));
if(_strip > 0 && _digits > 0)
  _frames = string_digits(_file);
else
  _frames=get_integer("Is this a sprite strip?  If so, how many frames? If not, leave as 0.  Gifs will only load the first frame.",0);
if(_frames<0)
    _frames=0;
var _remove_back=!show_question("Is the background transparent?");
